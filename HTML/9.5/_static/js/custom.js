...
function add_version_selector() {
  return fetch("/docs/open/versionlist.txt")
    .then((res) => res.text())
    .then((text) => {
      ...
      const current = match ? match[1] : "master";
      const isValid = /^([0-9]+\.[0-9]+|master)$/.test(current);
      dropdown.value = isValid ? current : "master";
      dropdown.addEventListener("change", ver_sel);
    });
}

function ver_sel() {
  const v = document.getElementById("version_dropdown").value;
  window.location.href = v === "master"
    ? "https://lvgl.io/docs/open/"
    : "https://lvgl.io/docs/open/" + v + "/";
}
